...
This is the `tracker/` app within the `GameTracker` monorepo — see the
[repo root README](../README.md) for how it relates to `roomreset/`.
...
This SSHes into the Pi and runs the monorepo's `update.sh` there (`git pull`
if behind, `npm install` in whichever app(s) changed, restarts both pm2
services). See `CLAUDE.md` for the one-time Pi setup steps and full deploy
details.
