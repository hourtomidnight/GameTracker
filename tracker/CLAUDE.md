...
This app lives at `tracker/` inside the `GameTracker` monorepo (alongside
`roomreset/`). The Pi runs its own `git clone` of the whole monorepo at
`~/GameTracker` and self-updates — nothing is scp'd file-by-file anymore.

```bash
# push your commits to GitHub first, then, from tracker/:
npm run deploy
```

This SSHes in and runs `~/GameTracker/update.sh` on the Pi, which
`git fetch`s, does a fast-forward-only `git pull` if the Pi is behind
`origin/master`, runs `npm install` in whichever of `tracker/`/`roomreset/`
changed, then restarts both pm2 services. It refuses to auto-merge (fails
loudly instead) if the Pi's local copy has diverged — that needs a human to
look at it.

You can also SSH in and run `bash ~/GameTracker/update.sh` directly, or set
it up as a cron job on the Pi for auto-updates without a manual trigger.

**One-time setup on a fresh Pi** (see `../docs/PI-REBUILD.md` for the full
rebuild process): `git clone` the monorepo to `~/GameTracker`, `cd tracker
&& npm install`, add the gitignored `google-credentials.json` and
`data/password.txt` inside `tracker/`, then
`pm2 start server.js --name htm-server` (run from within `tracker/`, or
point pm2's `cwd` at it).
...
`google-credentials.json` (service account key) is required inside `tracker/`
on the Pi (`~/GameTracker/tracker/google-credentials.json`) but is **not**
committed — see `docs/GOOGLE-SHEETS-SETUP.md` for how it's
generated/installed. RoomReset reuses a copy of the same credentials file
inside `roomreset/` (same service account).
